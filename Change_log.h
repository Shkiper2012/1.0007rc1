...
xxx	(=)		xrGame\WeaponMagazinedWGrenade.cpp 				---> m_fZoomFactor = g_fov; 												// Karlan: сбрасываем зум. //
	...
	*/
	...
	(i)		xrGame\game_cl_base.cpp							---> Можно закомментировать для восстановления солнца. Нужно проверить!
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
	(-)		Вырезан 'xrGameSpy.dll' и часть мультиплеерного кода.
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	*/
	...
	======= build 6858 	======= дохнем при новой игре =======
	======= commit 16 	=======
	*/
	
	/*		18_11_2017
	(+) 	config\text\rus\ui_st_other.xml 			---> st_restart_simulator 	---> "Рестарт симуляции" ---> Вызывается при повторной загрузке уже загруженной локации.
	(-)		xrGame\game_sv_base.h(cpp) 					---> По капле "выжимаю" мультиплеерный код.
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
	(~)		xrGame\Grenade.cpp							---> Переключение гранат. 										// by NanoBot //
	(+)		xrGame\ui\UICarBodyWnd.cpp 					---> Перемещение всей стопки предметов по 'ctrl + db_click' 	// by UnLoaded & Charsi //
	(/)		xrGame\game_cl_base.cpp 					---> Закомментировано для восстановления Солнца. 				// by Krodin //
	(~) 	gamedata\scripts\bind_stalker.script		---> "self.weather_manager:reset()" перенесён в апдейт для восстановления Солнца. 
	(~)		xrGame\MainMenu.cpp							---> Формат вывода версии в гл.меню: "1.0007rc1, build 1000 (Nov 18 2017)"
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
	(-)		xrGame\Actor.cpp 							---> Убрал домножение 'FOV' на 'wpn_fov_ratio'. Интересно для чего это было нужно?
	(+)		xrGame\Weapon.cpp 							---> Добавил в конструктор, что сразу было нужное значение. 					---> m_fIronSightZoomFactor = g_fov;
	(~)		xrGame\WeaponMagazinedWGrenade.cpp 			---> В режиме подствольника, сбрасываем зум, как при открытой прицеле (мушке). 	---> m_fZoomFactor = m_fIronSightZoomFactor;
	(-)		xrGame\WeaponMagazined.cpp					---> Значению 'm_fIronSightZoomFactor' больше не устанавливается 'scope_zoom_factor'
														---> Теперь, если целимся через мушку, то "глаза не выпучиваем". 				---> fov не меняется!
														---> Если нужно поменять fov (при открытом прицеле) - в конфиге оружия нужно указывать: 
														---> ironsight_zoom_factor = 67.5; (значение 67.5 можно менять)
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
	======= build 6861 	======= дохнем при новой игре =======
	======= commit 17 	=======
	*/
	
	/*		29_11_2017
	(~)		xrGame\console_commands.cpp 		---> Консольные комманда включена в релизе: 'ph_gravity'.
	(/)		xrGame\ui\UIMainIngameWnd.cpp(h) 	---> Чтобы иконка "Неукротимого(череп в круге)" была только при режиме бога.
	(/)		Отключено, для теста магазина: 		---> AMMO_FROM_BELT ---> Патроны с пояса.
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
XXX	(~)		Вырезана ёще одна часть мультиплеерного кода. 	---> статистика для оружия и прочее.
			---> 
					Откатил эту правку!!! 
					Из-за того, что гг дохнул при старте новой игры. 
					Если грузить сейв, то всё нормально.
					Причины, мне пока не известны.
					Вернул изменения до коммиита 15 - именно для этой правки, остальные актуальны.
			<---
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
	======= build 6872 	=======
	======= commit 18 	=======
	...
	*/
	...
	/*		03_12_2017
	(~)		xrGame\ai_stalker_alife.cpp 			---> Ранг оружия для НПС всегда == 1; 	---> get_rank() ---> INFO: Можно написать свой менеджер оружия для НПС.
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
	>>> 	Выбор ответа в диалоге на кнопки 1-9 	// by Kondr48 //
	(+)		gamedata\config\ui\talk.xml 			---> нужно добавить <num_text ...>, иначе будут вылеты.
	(+)		gamedata\config\ui\talk_16.xml			---> нужно добавить <num_text ...>, иначе будут вылеты.
	(+)		xrGame\ui\UITalkWnd.h/cpp
	(+)		xrGame\ui\UITalkDialogWnd.h/cpp
	(+)		xrGame\ui\UIButton.h/cpp
	(+)		xrGame\PhraseDialog.h/cpp
	(+)		xrGame\Phrase.h/cpp
	<<< 	Ссылка: https://github.com/Kondr48/gz_team_engine/commit/8e5c8455dc5c90271d189176c07a94a50eadfd5a
			-------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
xxx	(-)		xrGame\script_engine.h(cpp) 	---> Убрал, ибо вылет при начале новой игры! ---> m_G_already_loaded ---> Чтобы повторно не загружать '_G.script'
	...
	*/
	...
	// Заметка: Чтобы версия менялась при новом билде - нужно каждый раз пересобирать библиотеку 'XR_3DA', именно в ней вычисляется номер билда.
	// xxx // 						-  (x)  - откат правки. 
	// XXX //						-  (X) 	- значительный откат == задействовано несколько файлов, серьёзное изменение.
