...
//#define LUAICP_COMPAT					// включает совместимость с перехватчиком Lua (luaicp.dll).
										// Задавать надо в свойствах: system.ltx ---> [actor_schedule] t_min = 0; t_max = 1; 
...
// Shkiper2012: AF_SHOW_DYNAMIC_PARAMS ---> Некорректно отображаются значения свойств в окне с описанием!
...
//#define RUCK_FLAG_PREFERRED			// позволяющий для всех предметов реагировать на флаг FRuckDefault. 
...
// Shkiper2012: LOCK_RELOAD_IN_SPRINT ---> Если начал перезаряжаться и сразу побежал, то это правка не сработает!
...
#define EAT_PORTIONS_INFLUENCE 			// Real Wolf: Уменьшаем вес и цену предмета после использования.
...
// ==================================== Правки от Shkiper2012 ======================================= 
#define NO_MOUSE_WHELL_SWITCH_SLOT		// Убирает возможность переключаться между слотами колесиком мыши.
#define NO_AUTO_RELOAD_WPN				// Блокируем авто-перезарядку и сброс зума, когда закончились патроны.
#define ONE_CLICK_ZOOM 					// Зум в один клик. Не нужно удерживать клавишу прицеливания.