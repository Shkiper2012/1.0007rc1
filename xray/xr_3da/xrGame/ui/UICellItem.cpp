...
void CUICellItem::UpdateConditionProgressBar()
{
    if(m_pParentList && m_pParentList->GetConditionProgBarVisibility())
    {
        ...
        if(pWeapon || pOutfit )
        {
			...
			float x = (itm_grid_size.x * (cellSizeX) - m_pConditionState->GetWidth()) * 0.5f;
            float y =  itm_grid_size.y * (cellSizeY) - m_pConditionState->GetHeight() - 1.0f;
