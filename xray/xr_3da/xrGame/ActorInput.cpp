...
#include "../../../build_config_defines.h" // for: INV_NEW_SLOTS_SYSTEM, NO_MOUSE_WHELL_SWITCH_SLOT
...
{
	// Меняем кратность зума, только если целимся или смотрим в бинокль. //
	if( IsZoomAimingMode() )
	{
		if( direction > 0 ){
			inventory().Action( kWPN_ZOOM_INC, CMD_START );
			return;
		}else{
			inventory().Action( kWPN_ZOOM_DEC, CMD_START );
			return;
		}
	} 

#ifndef NO_MOUSE_WHELL_SWITCH_SLOT
	if (direction>0)
		OnNextWeaponSlot();
	else
		OnPrevWeaponSlot();
